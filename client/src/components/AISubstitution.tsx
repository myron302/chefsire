...
/** Types returned by your mock API */
type Nutrition = {
  calories: number;
  fat: number;     // grams
  carbs: number;   // grams
...
export default function AISubstitution() {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/ingredients/ai-substitution?q=${encodeURIComponent(query.trim())}`);
      if (!res.ok) {
  ...
  const statBadge = (label: string, value: string | number, tone: "neutral" | "positive" | "negative" = "neutral") => {
                  ...
                  return (
                    <div key={idx} className="border border-gray-200 rounded-lg p-4 hover:shadow-sm transition-shadow">
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                        ...
                        {/* Instacart placeholder (wire later) */}
                        <div className="mt-2 md:mt-0">
                          <button
                            ...
                            onClick={() => {
                              ...
                              // window.open(`https://www.instacart.com/store?affiliate_id=YOUR_AFFILIATE_ID`, "_blank");
