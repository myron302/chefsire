...
test("the failure notice names the unsent message and says the live draft is unaffected", () => {
  ...
  assert.equal(composerForm.includes("typedText !== pending.text"), true);
...
test("a failed mark-read is recorded rather than retried from the effect", () => {
  ...
  assert.equal(mutation.includes("failCateringReadMark(state, attempt.lastReadMessageId)"), true);
  // Success takes the server's authoritative marker, which is monotonic, rather than the requested id.
  assert.equal(mutation.includes("completeCateringReadMark(state, body?.lastReadMessageId ?? null)"), true);
...
test("the read-marker retry is offered quietly and accessibly, and issues no request itself", () => {
  const retry = source.slice(source.indexOf("mayRetryCateringReadMark(ownReadMark, viewedId, unreadCount)"));
  ...
  assert.equal(button.includes("disabled={readPending}"), true);
  // The control only clears the recorded attempt; the effect then issues exactly one request.
  assert.equal(button.includes("retryCateringReadMark(hydrateCateringReadMark(current, identity))"), true);
...
test("the read boundary comes from what was displayed, never from the newest fetched message", () => {
  ...
  assert.equal(source.includes("mayRetryCateringReadMark(ownReadMark, viewedId, unreadCount)"), true);
});
...
test("a sentinel at the end of the thread is what advances the viewed boundary", () => {
  ...
  assert.equal(viewEffect.includes("coverageObserver.disconnect();"), true);
...
test("both observers watch the SENTINEL, differing only in their root", () => {
  // Three: the bottom sentinel against both roots, plus one observer for the messages themselves.
  assert.equal((viewEffect.match(/new IntersectionObserver/g) ?? []).length, 3);
  ...
  // Each rendered message is observed against the DOCUMENT root, which is clipped by every scrollable ancestor --
  // so a message reported there was visible through the thread container as well as on screen.
  assert.equal(viewEffect.includes("messageNodes.current.forEach((node) => coverageObserver.observe(node));"), true);
  assert.equal(viewEffect.includes("recordCateringMessageCoverage(current, generation, seen)"), true);
...
test("the boundary advances from the conjunction, and refuses when it does not hold", () => {
  ...
  assert.equal(conjunction.includes("if (!mayRecordCateringViewedBoundary(visibility, generation, hasOlderPages, unreadStart, messages)) return;"), true);
...
test("the viewed boundary is gated on pagination being exhausted", () => {
  ...
  const conjunction = source.slice(source.indexOf("// The boundary advances only while BOTH observations hold"), source.indexOf("// Marking read happens at most ONCE per boundary"));
  assert.equal(conjunction.includes("mayRecordCateringViewedBoundary(visibility, generation, hasOlderPages, unreadStart, messages)"), true);
 ...
 */
test("each rendered message is observable, and the registry is bounded to what is rendered", () => {
  // The message element itself is the observation target -- no extra nodes, so no accessibility surface is added.
  assert.equal(source.includes("data-message-id={item.id}"), true);
  assert.equal(source.includes("if (node) messageNodes.current.set(item.id, node); else messageNodes.current.delete(item.id);"), true);
  // Registered on mount and dropped on unmount, so only loaded pages are ever observed and no detached node is
  // retained; the observer itself is disconnected with the others.
  assert.equal(viewEffect.includes("coverageObserver.disconnect();"), true);
  assert.equal(source.includes("const messageNodes = useRef(new Map<string, HTMLElement>());"), true);
});
