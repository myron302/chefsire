...
import type { StoreLayoutCustomizationConfig, StoreThemeTokens, FontPairingId, ButtonShape, CornerRadius } from "@shared/store/storeLayout";
import { FONT_PAIRINGS } from "@shared/store/storeLayout";
import { STORE_THEME_LIST, STORE_THEME_PRESETS, resolveThemeTokens } from "@shared/store/storeThemes";
...
function makeInitialCustomization(saved: StoreLayoutCustomizationConfig): StoreLayoutCustomizationConfig {
  return {
    logo: saved?.logo || "",
    ...
    socialLinks: saved?.socialLinks || { instagram: "", facebook: "", twitter: "", email: "", phone: "" },
...
export default function UnifiedStoreCustomizer({ store, products, onSaved }: UnifiedStoreCustomizerProps) {
  ...
  const isDirty = useMemo(() => {
    ...
    return JSON.stringify(draftCustomization) !== JSON.stringify(makeInitialCustomization(savedCustomization));
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Strip legacy `colors` — all color overrides are now in `tokens`
  ...
  // Resolve effective tokens so color pickers show preset value when no override exists
  const resolvedTokens = resolveThemeTokens(draftTheme, c.tokens);
  const presetTokens = STORE_THEME_PRESETS[draftTheme] ?? STORE_THEME_PRESETS.modern;

  // Helper: effective value for a color token (override ?? preset)
    ...
    return (
      ...
      >
        ...
        <div className="px-1.5 py-1" style={{ backgroundColor: preset.surface }}>
          <p className="text-[10px] font-semibold leading-tight truncate" style={{ color: preset.text }}>
            {name}
          </p>
          <div className="flex gap-0.5 mt-0.5">
            {[preset.primary, preset.secondary, preset.accent].map((col) => (
              <div key={col} className="w-2.5 h-2.5 rounded-full border border-black/10" style={{ backgroundColor: col }} />
            ))}
          </div>
        </div>
      </button>
    ...
    <div className="flex h-[calc(100vh-200px)] min-h-[600px]">
      ...
      <div className="w-[420px] flex-shrink-0 flex flex-col border-r bg-white overflow-hidden">
          ...
          <Accordion type="multiple" defaultValue={["theme", "branding"]} className="space-y-1">
            ...
            <AccordionItem value="theme" className="border rounded-lg px-3">
              ...
              <AccordionContent className="pb-4 space-y-3">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-gray-400 mb-1.5 font-semibold">Originals</p>
                  <div className="grid grid-cols-2 gap-2">
                    {ORIGINALS_LIST.map((t) => <ThemeCard key={t.id} id={t.id} name={t.name} />)}
                  </div>
                </div>
                <div className="border-t pt-3">
                  <p className="text-[10px] uppercase tracking-widest text-gray-400 mb-1.5 font-semibold">Curated</p>
                  <div className="grid grid-cols-2 gap-2">
                    {CURATED_LIST.map((t) => <ThemeCard key={t.id} id={t.id} name={t.name} />)}
                  </div>
                </div>
              </AccordionContent>
            </AccordionItem>
            ...
            <AccordionItem value="branding" className="border rounded-lg px-3">
              ...
              <AccordionContent className="pb-4 space-y-4">
                <div>
                  ...
                  <Input value={draftName} onChange={(e) => setDraftName(e.target.value)} className="mt-1 text-sm" placeholder="Your store name" />
                </div>
                <div>
                  <Label className="text-xs">Bio / Description</Label>
                  <Textarea value={draftBio} onChange={(e) => setDraftBio(e.target.value)} rows={3} className="mt-1 text-sm resize-none" placeholder="Tell shoppers what your store is about…" />
                </div>
                <div>
                  ...
                  {c.logo ? (
                    <div className="relative inline-block mt-1">
                      ...
                      <button onClick={() => patchCustomization({ logo: "" })} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-0.5 hover:bg-red-600">
                        <X size={12} />
                      </button>
                    </div>
                  ) : (
                    <label className="mt-1 flex flex-col items-center justify-center w-full h-20 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                      ...
                      <input type="file" className="hidden" accept="image/*" onChange={(e) => handleImageUpload(e, "logo")} disabled={uploading} />
                    </label>
                  )}
                </div>
              </AccordionContent>
            ...
            {/* 3. Colors — 5 pickers, per-channel token overrides */}
            <AccordionItem value="colors" className="border rounded-lg px-3">
              ...
              <AccordionContent className="pb-4 space-y-3">
                <p className="text-xs text-gray-500">Override individual colors. Others continue tracking the theme preset.</p>
                <ColorPicker label="Primary" value={colorVal("primary")}
                  onChange={(v) => patchToken("primary", v)} />
                <ColorPicker label="Secondary" value={colorVal("secondary")}
                  onChange={(v) => patchToken("secondary", v)} />
                <ColorPicker label="Accent" value={colorVal("accent")}
                  onChange={(v) => patchToken("accent", v)} />
                <ColorPicker label="Surface (background)" value={colorVal("surface")}
                  onChange={(v) => patchToken("surface", v)} />
                <ColorPicker label="Text" value={colorVal("text")}
                  onChange={(v) => patchToken("text", v)} />
                {c.tokens && Object.keys(c.tokens).some((k) => ["primary","secondary","accent","surface","text"].includes(k)) && (
                  <button
                    onClick={() => {
                      const { primary: _p, secondary: _s, accent: _a, surface: _su, text: _t, ...rest } = c.tokens ?? {};
            ...
            <AccordionItem value="typography" className="border rounded-lg px-3">
              ...
              <AccordionContent className="pb-4 space-y-3">
                ...
                <div className="border rounded-lg p-3 bg-gray-50 space-y-1">
                  ...
                  <p className="font-bold text-sm" style={{ fontFamily: FONT_PAIRINGS[fontPairingVal ?? presetTokens.fontPairing]?.heading }}>
                    The quick brown fox
                  </p>
                  <p className="text-xs" style={{ fontFamily: FONT_PAIRINGS[fontPairingVal ?? presetTokens.fontPairing]?.body }}>
              ...
              <AccordionContent className="pb-4 space-y-3">
                ...
                <div className="flex gap-2 mt-2">
                  {BUTTON_SHAPE_OPTIONS.map((opt) => (
                    <div
                      key={opt.value}
                      className={`px-4 py-1.5 text-xs border-2 border-orange-400 text-orange-600 font-medium transition-colors ${
                        (buttonShapeVal ?? presetTokens.buttonShape) === opt.value ? "opacity-100" : "opacity-30"
                      }`}
                      style={{
                        borderRadius:
                  ...
                  {CORNER_RADIUS_OPTIONS.map((opt) => (
                    <div
                      ...
                      className={`w-10 h-10 bg-orange-400 transition-opacity ${
                        (cornerRadiusVal ?? presetTokens.cornerRadius) === opt.value ? "opacity-100" : "opacity-30"
                      }`}
                      style={{
                        borderRadius: opt.value === "sharp" ? "0px" : opt.value === "round" ? "16px" : "8px",
                      }}
                    />
                  ))}
                </div>
            ...
            <AccordionItem value="banner" className="border rounded-lg px-3">
              <AccordionTrigger className="text-sm font-semibold py-3">
                <span className="flex items-center gap-2">
                  Hero / Banner
                  <Switch checked={c.showBanner !== false} onCheckedChange={(v) => patchCustomization({ showBanner: v })} onClick={(e) => e.stopPropagation()} className="scale-75" />
                </span>
              </AccordionTrigger>
              {c.showBanner !== false && (
                <AccordionContent className="pb-4 space-y-3">
                  {c.bannerImage ? (
                    <div className="relative">
                      ...
                      <button onClick={() => patchCustomization({ bannerImage: "" })} className="absolute top-1.5 right-1.5 bg-red-500 text-white rounded-full p-0.5 hover:bg-red-600">
                        <X size={12} />
                      </button>
                    </div>
                  ) : (
                    <label className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                      ...
                      <input type="file" className="hidden" accept="image/*" onChange={(e) => handleImageUpload(e, "bannerImage")} disabled={uploading} />
                    </label>
                  )}
                  <div>
                    <Label className="text-xs">Banner Title</Label>
                    <Input value={c.bannerTitle || ""} onChange={(e) => patchCustomization({ bannerTitle: e.target.value })} placeholder="Welcome to our store!" className="mt-1 text-sm" />
                  </div>
                  <div>
                    <Label className="text-xs">Banner Subtitle</Label>
                    <Input value={c.bannerSubtitle || ""} onChange={(e) => patchCustomization({ bannerSubtitle: e.target.value })} placeholder="Discover amazing products" className="mt-1 text-sm" />
                  </div>
                </AccordionContent>
              )}
            </AccordionItem>
            ...
            <AccordionItem value="about" className="border rounded-lg px-3">
              <AccordionTrigger className="text-sm font-semibold py-3">
                <span className="flex items-center gap-2">
                  About
                  <Switch checked={c.aboutEnabled || false} onCheckedChange={(v) => patchCustomization({ aboutEnabled: v })} onClick={(e) => e.stopPropagation()} className="scale-75" />
                </span>
              </AccordionTrigger>
              {c.aboutEnabled && (
                <AccordionContent className="pb-4 space-y-3">
                  <div>
                    ...
                    <Input value={c.aboutTitle || "About Us"} onChange={(e) => patchCustomization({ aboutTitle: e.target.value })} className="mt-1 text-sm" />
                  </div>
                  <div>
                    <Label className="text-xs">Content</Label>
                    <Textarea value={c.aboutContent || ""} onChange={(e) => patchCustomization({ aboutContent: e.target.value })} rows={4} className="mt-1 text-sm resize-none" placeholder="Tell your story…" />
                  </div>
                </AccordionContent>
              )}
            </AccordionItem>
            ...
            <AccordionItem value="announcement" className="border rounded-lg px-3">
              <AccordionTrigger className="text-sm font-semibold py-3">
                <span className="flex items-center gap-2">
                  Announcement Bar
                  <Switch checked={c.announcementEnabled || false} onCheckedChange={(v) => patchCustomization({ announcementEnabled: v })} onClick={(e) => e.stopPropagation()} className="scale-75" />
                </span>
              </AccordionTrigger>
              {c.announcementEnabled && (
                <AccordionContent className="pb-4">
                  <Input value={c.announcementBar || ""} onChange={(e) => patchCustomization({ announcementBar: e.target.value })} placeholder="e.g., Free shipping on orders over $50!" className="text-sm" />
                </AccordionContent>
              )}
            </AccordionItem>
              ...
              <AccordionContent className="pb-4 space-y-4">
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-1.5 mt-1.5">
                    {[2, 3, 4, 5].map((cols) => (
                      <button key={cols} onClick={() => patchCustomization({ layout: { ...c.layout, gridColumns: cols } })}
                        className={`py-2 border-2 rounded-lg text-center text-xs font-semibold transition-colors ${c.layout?.gridColumns === cols ? "border-orange-500 bg-orange-50 text-orange-700" : "border-gray-200 hover:border-gray-300"}`}>
                        {cols}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-1.5">
                    {[{ value: "elevated", label: "Elevated", desc: "With shadow" }, { value: "flat", label: "Flat", desc: "Simple border" }].map((opt) => (
                      <button key={opt.value} onClick={() => patchCustomization({ layout: { ...c.layout, productCardStyle: opt.value as any } })}
                        className={`p-3 border-2 rounded-lg transition-colors text-left ${c.layout?.productCardStyle === opt.value ? "border-orange-500 bg-orange-50" : "border-gray-200 hover:border-gray-300"}`}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-1.5 mt-1.5">
                    {["compact", "normal", "relaxed"].map((sp) => (
                      <button key={sp} onClick={() => patchCustomization({ layout: { ...c.layout, spacing: sp as any } })}
                        className={`py-2 border-2 rounded-lg text-center text-xs font-medium capitalize transition-colors ${c.layout?.spacing === sp ? "border-orange-500 bg-orange-50 text-orange-700" : "border-gray-200 hover:border-gray-300"}`}>
                        {sp}
                      </button>
                    ))}
                  </div>
              ...
              <AccordionContent className="pb-4 space-y-3">
                {[
                  ...
                  <div key={key}>
                    ...
                    <Input type={type || "text"} value={(c.socialLinks as any)?.[key] || ""} onChange={(e) => patchCustomization({ socialLinks: { ...c.socialLinks, [key]: e.target.value } })} placeholder={placeholder} className="mt-1 text-xs" />
                  </div>
                ))}
                <div>
                  <Label className="flex items-center gap-1.5 text-xs"><MapPin size={13} /> Address</Label>
                  <Textarea value={c.contactInfo?.address || ""} onChange={(e) => patchCustomization({ contactInfo: { ...c.contactInfo, address: e.target.value } })} rows={2} className="mt-1 text-xs resize-none" placeholder="123 Main St, City, State" />
                </div>
                <div>
                  <Label className="text-xs">Store Hours</Label>
                  <Textarea value={c.contactInfo?.hours || ""} onChange={(e) => patchCustomization({ contactInfo: { ...c.contactInfo, hours: e.target.value } })} rows={2} className="mt-1 text-xs resize-none" placeholder={"Mon–Fri: 9am–6pm\nSat–Sun: 10am–4pm"} />
                </div>
              </AccordionContent>
            </AccordionItem>
          </Accordion>
        ...
        <div className="flex-shrink-0 border-t bg-white px-4 py-3 flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={handleDiscard} disabled={!isDirty || saving}>
            <RotateCcw className="w-3.5 h-3.5 mr-1.5" />
            Discard
          </Button>
          <Button size="sm" onClick={handleSave} disabled={!isDirty || saving} className="bg-orange-500 hover:bg-orange-600 text-white">
