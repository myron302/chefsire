...
export default function DessertSmoothiesPage() {
  ...
  const [activeTab, setActiveTab] = useState<"browse" | "dessert-types" | "categories" | "featured">(
    "browse"
  );
  ...
  const handleShareSmoothie = async (smoothie: any, servingsOverride?: number) => {
    ...
    const servings =
      servingsOverride ?? servingsById[smoothie.id] ?? (smoothie.recipe?.servings || 1);
    ...
    const text = `${smoothie.name} • ${smoothie.dessertType || ""} • ${smoothie.bestTime || ""}\n${preview}${
      (smoothie.ingredients || []).length > 4
        ? ` …plus ${(smoothie.ingredients || []).length - 4} more`
        : ""
    }`;
  ...
  const getFilteredSmoothies = () => {
    let filtered = smoothieRecipesWithMeasurements.filter((smoothie: any) => {
      ...
      const matchesSearch =
        smoothie.name?.toLowerCase().includes(q) ||
        smoothie.description?.toLowerCase().includes(q);
      const matchesType = !selectedDessertType || smoothie.dessertType === selectedDessertType;
      const matchesCategory =
        !selectedCategory ||
        (Array.isArray(smoothie.category)
          ? smoothie.category.includes(selectedCategory)
          : smoothie.category === selectedCategory);
      const cal = smoothie.nutrition?.calories ?? 0;
      const addedSugar = smoothie.nutrition?.added_sugar ?? null;
      const matchesCalories = maxCalories === "all" || cal <= (maxCalories as number);
      const matchesSweetener = !onlyNaturalSweetener || addedSugar === 0;
      ...
      {selectedRecipe && (
        <RecipeKit
          ...
          item={{
            ...
            defaultServings:
              servingsById[selectedRecipe.id] ?? selectedRecipe.recipe?.servings ?? 1,
          }}
        />
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <Card className="bg-gradient-to-r from-pink-50 to-purple-50 border-pink-200">
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
              {smoothieSubcategories.map((subcategory: any) => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-pink-600">
                {Math.round(
                  filteredSmoothies.reduce(
                    (acc: number, s: any) => acc + (s.nutrition?.calories || 0),
                    0
                  ) / Math.max(1, filteredSmoothies.length)
                ) || 0}
              </div>
          ...
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-pink-600">
                {Math.round(
                  filteredSmoothies.reduce(
                    (acc: number, s: any) => acc + (s.nutrition?.protein || 0),
                    0
                  ) / Math.max(1, filteredSmoothies.length)
                ) || 0}
                g
          ...
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-pink-600">
                {(
                  filteredSmoothies.reduce((acc: number, s: any) => acc + (s.rating || 0), 0) /
                  Math.max(1, filteredSmoothies.length)
                ).toFixed(1)}
                ★
              </div>
        ...
        <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
          {[
            ...
            { id: "dessert-types", label: "Dessert Types", icon: Star },
                      ...
                      <div className="grid grid-cols-3 gap-2 mb-4 text-center text-sm">
                        <div>
                          <div className="font-bold text-pink-600">
                            {smoothie.nutrition?.calories ?? 0}
                          </div>
                          <div className="text-gray-500">Cal</div>
                        </div>
                        <div>
                          <div className="font-bold text-pink-600">
                            {smoothie.nutrition?.protein ?? 0}g
                          </div>
                          ...
                          <div className="mb-4 bg-gray-50 border border-gray-200 rounded-lg p-3">
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    setServingsById((prev) => ({
                                      ...prev,
                                      [smoothie.id]: clamp(
                                        (prev[smoothie.id] ?? (smoothie.recipe?.servings || 1)) - 1
                                      ),
                                    }))
                                  }
                            ...
                            <div className="flex gap-2 mt-3">
                              ...
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => handleShareSmoothie(smoothie, servings)}
                              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {featuredSmoothies.map((smoothie: any) => (
              <Card key={smoothie.id} className="overflow-hidden hover:shadow-xl transition-shadow">
                ...
                <CardContent>
                  <Button className="w-full bg-pink-600 hover:bg-pink-700" onClick={() => openRecipeModal(smoothie)}>
                    <IceCream className="h-4 w-4 mr-2" />
                    Make This Dessert
                  </Button>
                </CardContent>
