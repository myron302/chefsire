...
import { 
  ...
  Activity, BarChart3, Crown, Dumbbell, Eye, Bone
} from 'lucide-react';
...
export default function CollagenProteinPage() {
  ...
  const handleMakeShake = (shake: any) => {
    ...
    console.log(`Made ${shake.name}! +35 XP`);
  };
                  ...
                  <CardContent>
                    <div className="space-y-3 mb-4">
                      ...
                      <div className="bg-purple-50 p-3 rounded-lg">
                        ...
                        <div className="text-sm text-purple-800">{source.bestFor}</div>
                      </div>
                    </div>
        ...
        {activeTab === 'beauty-goals' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {beautyGoals.map(goal => {
              ...
              const goalShakes = collagenShakes.filter(shake => 
                shake.primaryBenefit.toLowerCase().includes(goal.name.toLowerCase().split(' ')[0])
              );
