      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              {sisterPetFoodPages.map((page) => {
                ...
                return (
                  <Link key={page.id} href={page.path}>
                    <Button variant="outline" className="w-full justify-start hover:bg-cyan-50 hover:border-cyan-300 h-auto py-4">
                      <Icon className="h-6 w-6 mr-3 text-cyan-600" />
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <Input placeholder="Search bird food recipes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 h-12 text-base" />
          </div>
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-1 mb-3">
                    {recipe.badges.slice(0, 2).map((badge: string) => (
                      <Badge key={`top-${badge}`} className="bg-cyan-600 text-white text-xs">{badge}</Badge>
                    ))}
                  </div>
                  {/* Nutrition Grid */}
                  ...
                  {/* Action - BIRD ICON */}
                  <div className="mt-3">
                    <Button className="w-full bg-cyan-600 hover:bg-cyan-700" onClick={(event) => {
                      event.stopPropagation();
                      handleRecipeCardNavigation(recipe);
                    }}>
