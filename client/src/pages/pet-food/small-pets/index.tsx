      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              {sisterPetFoodPages.map((page) => {
                ...
                return (
                  <Link key={page.id} href={page.path}>
                    <Button variant="outline" className="w-full justify-start hover:bg-emerald-50 hover:border-emerald-300 h-auto py-4">
                      <Icon className="h-6 w-6 mr-3 text-emerald-600" />
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <Input placeholder="Search small pet food recipes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 h-12 text-base" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRecipes.map(recipe => {
            ...
            const recipeRoute = getCanonicalFirstPath({
              recipeName: recipe.name,
              fallbackPath: '/pet-food/small-pets',
              resolveCanonicalSlug: resolveCanonicalPetFoodSlug,
              recipeBasePath: '/pet-food/recipe',
            });
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-1 mb-3">
                    {recipe.badges.slice(0, 2).map((badge: string) => (
                      <Badge key={`top-${badge}`} className="bg-emerald-600 text-white text-xs">{badge}</Badge>
                    ))}
                  </div>
                  {/* Nutrition Grid */}
