...
export default function CateringProviderPage() {
  ...
  return <main className="mx-auto max-w-6xl px-3 py-6 sm:px-4 sm:py-10">
    ...
    {section === "inquiries" && <InquiryManager providerId={user.id} openBookings={() => selectSection("bookings")} />}
...
type Inquiry = { id: string; status: string | null; eventDate: string; eventType: string | null; guestCount: number | null; message: string | null; booking: InquiryBookingProjection | null };
function InquiryManager({ providerId, openBookings }: { providerId: string; openBookings: () => void }) {
  ...
  return <Card><CardHeader><CardTitle>Inquiries</CardTitle><CardDescription>Customer quote requests. Accepted requests are not described as paid or confirmed bookings.</CardDescription></CardHeader><CardContent>{query.isLoading ? <p role="status">Loading inquiries…</p> : query.isError ? <div role="alert" className="space-y-3"><p>Inquiries could not be loaded.</p><Button variant="outline" onClick={() => query.refetch()}>Retry</Button></div> : inquiries?.length ? <div className="space-y-3">{inquiries.map((item) => <article key={item.id} className="min-w-0 rounded-lg border p-4"><div className="flex flex-wrap justify-between gap-2"><h2 className="break-words font-semibold">{item.eventType || "Catering request"}</h2><span className="rounded-full border px-2 py-1 text-xs capitalize">{item.status || "pending"}</span></div><p className="mt-2 text-sm">Requested event date: {formatCateringCalendarDate(item.eventDate)}{item.guestCount ? ` · ${item.guestCount} guests` : ""}</p>{item.message && <p className="mt-2 whitespace-pre-wrap break-words text-sm text-muted-foreground">{item.message}</p>}{item.status === "accepted" && <InquiryBookingAction inquiry={item} offer={offer} openBookings={openBookings} />}{item.status === "pending" && <div className="mt-3 flex flex-wrap gap-2"><Button disabled={mutation.isPending} onClick={() => mutation.mutate({ id: item.id, status: "accepted" })}>Accept request</Button><Button disabled={mutation.isPending} variant="outline" onClick={() => mutation.mutate({ id: item.id, status: "declined" })}>Decline request</Button></div>}</article>)}</div> : <p className="rounded-lg border border-dashed p-6 text-center text-muted-foreground">No inquiries have been received yet.</p>}{(mutation.isError || offer.isError) && <p className="mt-3 text-destructive" role="alert">{mutation.error?.message || offer.error?.message}</p>}{pagination && pagination.totalPages > 0 && <nav aria-label="Inquiry pages" className="mt-5 flex flex-wrap items-center justify-between gap-3"><Button className="min-h-11" variant="outline" disabled={page <= 1 || query.isFetching} onClick={() => setPage((current) => Math.max(1, current - 1))}>Previous</Button><p className="text-sm" aria-live="polite">Page {pagination.page} of {pagination.totalPages}</p><Button className="min-h-11" variant="outline" disabled={page >= pagination.totalPages || query.isFetching} onClick={() => setPage((current) => current + 1)}>Next</Button></nav>}</CardContent></Card>;
}

function InquiryBookingAction({ inquiry, offer, openBookings }: { inquiry: Inquiry; offer: UseMutationResult<{ booking: InquiryBookingProjection }, Error, string>; openBookings: () => void }) {
  const state = inquiryBookingPresentation(inquiry.booking);
  const pending = offer.isPending && offer.variables === inquiry.id;
  if (state.canOffer) return <div className="mt-3"><Button className="min-h-11" disabled={pending} onClick={() => offer.mutate(inquiry.id)}>{pending ? "Offering booking terms…" : state.label}</Button></div>;
  return <div className="mt-3 flex flex-wrap items-center gap-2"><span className="text-sm font-medium" role="status">{state.label}</span><Button className="min-h-11" variant="outline" onClick={openBookings}>View booking</Button></div>;
}
