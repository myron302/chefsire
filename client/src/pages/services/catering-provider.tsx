...
function InquiryManager({ providerId }: { providerId: string }) {
  ...
  const key = ["catering", "inquiries", providerId] as const;
  const query = useQuery({ queryKey: key, queryFn: async (): Promise<Inquiry[]> => { const response = await fetch(`/api/catering/users/${providerId}/inquiries`, { credentials: "include" }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.message || "Inquiries could not be loaded"); return body.inquiries; }, staleTime: 60_000, refetchOnWindowFocus: true });
  const mutation = useMutation({ mutationFn: async ({ id, status }: { id: string; status: "accepted" | "declined" }) => { const response = await fetch(`/api/catering/inquiries/${id}`, { method: "PUT", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) }); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.message || "Inquiry could not be updated"); }, onSuccess: async () => { await Promise.all([client.invalidateQueries({ queryKey: key }), client.invalidateQueries({ queryKey: cateringDashboardKey(providerId) })]); } });
  return <Card><CardHeader><CardTitle>Inquiries</CardTitle><CardDescription>Customer quote requests. Accepted requests are not described as paid or confirmed bookings.</CardDescription></CardHeader><CardContent>{query.isLoading ? <p role="status">Loading inquiries…</p> : query.isError ? <div role="alert" className="space-y-3"><p>Inquiries could not be loaded.</p><Button variant="outline" onClick={() => query.refetch()}>Retry</Button></div> : query.data?.length ? <div className="space-y-3">{query.data.map((item) => <article key={item.id} className="min-w-0 rounded-lg border p-4"><div className="flex flex-wrap justify-between gap-2"><h2 className="break-words font-semibold">{item.eventType || "Catering request"}</h2><span className="rounded-full border px-2 py-1 text-xs capitalize">{item.status || "pending"}</span></div><p className="mt-2 text-sm">Requested event date: {new Date(item.eventDate).toLocaleDateString()}{item.guestCount ? ` · ${item.guestCount} guests` : ""}</p>{item.message && <p className="mt-2 whitespace-pre-wrap break-words text-sm text-muted-foreground">{item.message}</p>}{item.status === "pending" && <div className="mt-3 flex flex-wrap gap-2"><Button disabled={mutation.isPending} onClick={() => mutation.mutate({ id: item.id, status: "accepted" })}>Accept request</Button><Button disabled={mutation.isPending} variant="outline" onClick={() => mutation.mutate({ id: item.id, status: "declined" })}>Decline request</Button></div>}</article>)}</div> : <p className="rounded-lg border border-dashed p-6 text-center text-muted-foreground">No inquiries have been received yet.</p>}{mutation.isError && <p className="mt-3 text-destructive" role="alert">{mutation.error.message}</p>}</CardContent></Card>;
}
