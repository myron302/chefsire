...
import { PackageGallery } from "@/components/catering/PackageGallery";
...
export default function CateringPublicProvider({ params }: { params: { providerId: string } }) {
  ...
  const [selectedPackage, setSelectedPackage] = useState<CateringPackage>();
  ...
  const packagesQuery = useQuery({ queryKey: ["catering", "packages", params.providerId], queryFn: async (): Promise<CateringPackage[]> => { const response = await fetch(`/api/catering/providers/${encodeURIComponent(params.providerId)}/packages`); if (!response.ok) throw new Error("Unable to load packages"); return (await response.json()).packages; }, enabled: providerQuery.isSuccess, staleTime: 5 * 60 * 1000 });
  const dateAvailabilityQuery = useQuery({ queryKey: ["catering", "date-availability", params.providerId, quote.eventDate], queryFn: async (): Promise<{ available: boolean; reason: string }> => { const response = await fetch(`/api/catering/providers/${encodeURIComponent(params.providerId)}/availability?date=${encodeURIComponent(quote.eventDate)}`); const body = await response.json().catch(() => ({})); if (!response.ok) throw new Error(body.message || "Unable to check this date"); return body; }, enabled: quoteOpen && Boolean(quote.eventDate), staleTime: 30_000 });
  ...
  const requestQuote = (item?: CateringPackage) => { if (requireSignIn()) { setSelectedPackage(item); setQuoteOpen(true); } };
  ...
  return <div className="mx-auto max-w-5xl px-4 pb-28 pt-6 sm:pb-10 sm:pt-8">
    ...
    <div className="mt-6 grid gap-6 md:grid-cols-3"><Card className="md:col-span-2"><CardHeader><CardTitle>About this catering service</CardTitle></CardHeader><CardContent>{provider.cateringBio ? <p className="whitespace-pre-wrap leading-7 text-muted-foreground">{provider.cateringBio}</p> : <div className="rounded-lg border border-dashed p-6 text-center text-muted-foreground">This provider has not added a catering bio yet.</div>}</CardContent></Card>
      <Card><CardHeader><CardTitle>Service details</CardTitle></CardHeader><CardContent className="space-y-5"><div className="flex gap-3"><MapPin className="mt-0.5 h-5 w-5 shrink-0 text-orange-600" /><div><p className="font-medium">Service location</p><p className="text-sm text-muted-foreground">{provider.cateringLocation || "Not provided"}</p></div></div><div className="flex gap-3"><Navigation className="mt-0.5 h-5 w-5 shrink-0 text-orange-600" /><div><p className="font-medium">Travel radius</p><p className="text-sm text-muted-foreground">{provider.cateringRadius == null ? "Not provided" : `Up to ${provider.cateringRadius} miles`}</p></div></div><div className="flex gap-3"><Users className="mt-0.5 h-5 w-5 shrink-0 text-orange-600" /><div><p className="font-medium">Availability</p><p className="text-sm text-muted-foreground">{provider.availability?.acceptingBookings ? "Accepting event inquiries" : "Not accepting inquiries right now"}</p>{provider.availability?.earliestInquiryDate && <p className="text-xs text-muted-foreground">Inquiry window: {provider.availability.earliestInquiryDate} through {provider.availability.latestInquiryDate}</p>}</div></div></CardContent></Card>
    </div>
    ...
    <Card className="mt-6"><CardHeader><CardTitle>Packages &amp; menus</CardTitle></CardHeader><CardContent>{packagesQuery.isLoading ? <div role="status">Loading packages…</div> : packagesQuery.isError ? <p className="text-muted-foreground">Packages could not be loaded.</p> : <PackageGallery packages={packagesQuery.data ?? []} requestQuote={requestQuote} />}</CardContent></Card>
    <Card className="mt-6"><CardContent className="p-6"><CateringReviews providerId={provider.id} /></CardContent></Card>
    <Card className="mt-6 hidden sm:block"><CardContent className="flex flex-wrap items-center justify-between gap-4 p-6"><div><p className="font-semibold">Planning an event?</p><p className="text-sm text-muted-foreground">{actionState.isAuthLoading ? "Checking your account…" : "Send real event details or begin a private conversation."}</p></div><div className="flex gap-3"><Button variant="outline" onClick={message} disabled={actionState.isAuthLoading || messageMutation.isPending || actionState.isSelf}><MessageCircle className="mr-2 h-4 w-4" />{actionState.isAuthLoading ? "Checking…" : "Message"}</Button><Button onClick={() => requestQuote()} disabled={actionState.isAuthLoading || !provider.availability?.acceptingBookings || actionState.isSelf}><CalendarDays className="mr-2 h-4 w-4" />{actionState.isAuthLoading ? "Checking…" : "Request Quote"}</Button></div></CardContent></Card>

    <div className="fixed inset-x-0 bottom-0 z-40 border-t bg-background/95 p-3 pb-[max(.75rem,env(safe-area-inset-bottom))] shadow-[0_-4px_16px_rgba(0,0,0,.08)] backdrop-blur sm:hidden"><div className="mx-auto grid max-w-lg grid-cols-3 gap-2"><Button variant="outline" size="sm" onClick={share}><Share2 className="mr-1 h-4 w-4" />Share</Button><Button variant="outline" size="sm" onClick={message} disabled={actionState.isAuthLoading || messageMutation.isPending || actionState.isSelf}><MessageCircle className="mr-1 h-4 w-4" />{actionState.isAuthLoading ? "Checking…" : "Message"}</Button><Button size="sm" onClick={() => requestQuote()} disabled={actionState.isAuthLoading || !provider.availability?.acceptingBookings || actionState.isSelf}><CalendarDays className="mr-1 h-4 w-4" />{actionState.isAuthLoading ? "Checking…" : "Quote"}</Button></div></div>

    <Dialog open={quoteOpen} onOpenChange={(open) => { setQuoteOpen(open); if (!open) setSelectedPackage(undefined); }}><DialogContent><DialogHeader><DialogTitle>Request a quote from {provider.displayName}</DialogTitle><DialogDescription>{selectedPackage ? `Package selected: ${selectedPackage.title}. ` : ""}Share your event details. This sends a real inquiry to the provider.</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); if (!isAuthLoading && dateAvailabilityQuery.data?.available) quoteMutation.mutate(); }}><div className="grid gap-4 sm:grid-cols-2"><div><Label htmlFor="quote-date">Event date</Label><Input id="quote-date" type="date" min={provider.availability?.earliestInquiryDate || localCalendarDate()} max={provider.availability?.latestInquiryDate || undefined} required value={quote.eventDate} onChange={(e) => setQuote({ ...quote, eventDate: e.target.value })} />{dateAvailabilityQuery.isFetching && <p className="text-xs text-muted-foreground" role="status">Checking provider calendar…</p>}{dateAvailabilityQuery.isError && <div className="mt-1 flex items-center gap-2 text-xs text-red-700" role="alert"><span>We could not check this date.</span><Button type="button" size="sm" variant="outline" onClick={() => dateAvailabilityQuery.refetch()}>Retry</Button></div>}{dateAvailabilityQuery.data && <p className={`text-xs ${dateAvailabilityQuery.data.available ? "text-emerald-700" : "text-red-700"}`}>{dateAvailabilityQuery.data.available ? "Available for an inquiry" : "Unavailable for an inquiry on this date"}</p>}</div><div><Label htmlFor="quote-guests">Guest count</Label><Input id="quote-guests" type="number" min="1" value={quote.guestCount} onChange={(e) => setQuote({ ...quote, guestCount: e.target.value })} /></div></div><div><Label htmlFor="quote-type">Event type</Label><Input id="quote-type" placeholder="Wedding, corporate event, private dinner…" value={quote.eventType} onChange={(e) => setQuote({ ...quote, eventType: e.target.value })} /></div><div><Label htmlFor="quote-message">Event details</Label><Textarea id="quote-message" rows={4} placeholder="Location, menu preferences, dietary needs, and other helpful details" value={quote.message} onChange={(e) => setQuote({ ...quote, message: e.target.value })} /></div><Button className="w-full" type="submit" disabled={isAuthLoading || quoteMutation.isPending || !dateAvailabilityQuery.data?.available}>{quoteMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Send quote request</Button></form></DialogContent></Dialog>
  </div>;
}
