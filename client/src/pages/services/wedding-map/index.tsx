    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 relative h-[600px]">
          ...
          <Card className="h-full overflow-hidden">
            <MapView 
              ...
              markers={vendors} 
              onIdle={(map) => { 
                setMapBoundsCenter({ lat: map.getCenter().lat(), lng: map.getCenter().lng() }); 
                setShowSearchArea(true); 
              }}
              onMarkerClick={(m) => { 
                setSelectedVendor(m.vendor || m); 
        ...
        <Card className="h-[600px] flex flex-col">
          <CardHeader className="py-3 border-b"><CardTitle className="text-md">Vendors ({vendors.length})</CardTitle></CardHeader>
          ...
          {selectedVendor && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>{selectedVendor.address}</DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div className="flex gap-4">
                  <Button variant="outline" size="sm" onClick={() => toggleComparison(selectedVendor.id)}>
                    {comparisonIds.includes(selectedVendor.id) ? "Remove from Compare" : "Add to Compare"}
                  </Button>
                </div>
              </div>
            </>
