...
export default function Explore() {
  ...
  // Reset filters (toast confirm)
  const resetAll = useCallback(() => {
    toast({
      title: "Reset all filters?",
      description: "This will clear search, chips, and all advanced filters.",
      action: (
        <Button
          size="sm"
          onClick={() => {
  ...
  if (isLoading && !data) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-6 sticky-ancestor-fix">
        <Header
      ...
      <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
        <SheetContent side="bottom" className="max-h-[85vh] overflow-y-auto">
          <SheetHeader className="mb-4">
            <SheetTitle>Filters {filterCount > 0 ? `(${filterCount} active)` : ""}</SheetTitle>
          </SheetHeader>
          ...
          <div className="grid gap-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  className="w-32"
                />
                <Button variant="ghost" size="sm" onClick={() => setMaxCalories(null)} aria-label="Clear calories">
                  <X className="h-4 w-4" />
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <Label>Sort</Label>
              <select value={sort} onChange={(e) => setSort(e.target.value as SortKey)} className="h-9 rounded-md border bg-background px-2 text-sm w-full" aria-label="Sort posts">
    ...
    <div className="mb-4 sticky top-0 z-10 bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/70 py-4">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          {POPULAR_DIET_CHIPS.map((d) => {
            ...
            return (
              <Badge
                ...
                variant={active ? "default" : "outline"}
                className="cursor-pointer shrink-0"
                onClick={() => (active ? setDiets(diets.filter((x) => x !== d)) : setDiets([...diets, d]))}
                aria-pressed={active}
                title={`Diet: ${d}`}
        ...
        <div className="flex items-center gap-2 md:ml-auto">
          <Button variant="outline" size="sm" onClick={openFilters} aria-label={`Open filters (${filterCount} active)`}>
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        <select value={sort} onChange={(e) => setSort(e.target.value as SortKey)} className="h-9 rounded-md border bg-background px-2 text-sm" aria-label="Sort posts">
          <option value="trending">Trending</option>
          <option value="newest">Newest</option>
          <option value="most_liked">Most Liked</option>
        </select>
...
function RangeRow({
  ...
  return (
    <div className="space-y-2">
      ...
      <Slider value={value} min={0} max={max} step={5} onValueChange={(v) => onChange([v[0] ?? value[0], v[1] ?? value[1]])} className="w-full" />
    </div>
  );
}
