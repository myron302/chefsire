      ...
      {allPosts.length > 0 ? (
        viewMode === "grid" ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4" data-testid="grid-explore">
            {allPosts.map((post) => (
              <Card key={post.id} className="group cursor-pointer hover:shadow-lg transition-shadow overflow-hidden">
                ...
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <img src={post.user.avatar || ""} alt={post.user.displayName} className="w-6 h-6 rounded-full bg-muted" loading="lazy" decoding="async" />
          ...
          <div className="space-y-8 mt-4" data-testid="list-explore">
            {allPosts.map((post) => (post.isRecipe ? <RecipeCard key={post.id} post={post} /> : <PostCard key={post.id} post={post} />))}
          </div>
        )
      ) : (
