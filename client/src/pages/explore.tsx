...
any) {
  return (
    <div className="mb-4 sticky top-0 z-10 bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/70 py-4 sticky-ancestor-fix">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        {/* Category pills */}
        <div className="flex gap-2 overflow-x-auto no-scrollbar touch-pan-x edge-fade-x py-1 -mx-2 px-2 snap-x snap-mandatory">
          {CATEGORIES.map((category: string) => {
            const active = selectedCategory === category || (!selectedCategory && category === "All");
            return (
              <Badge
...
function EmptyState({
  onClear, query, category,
}: { onClear: () => void; query: string; category: string | null }) {
  return (
    <div className="text-center py-16">
      <h3 className="text-lg font-semibold mb-2">No posts found</h3>
      <p className="text-muted-foreground max-w-md mx-auto">
