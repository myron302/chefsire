...
r.post("/inquiries/:inquiryId/provider-confirm", requireAuth, async (req, res, next) => { try {
  ...
  const result = await db.transaction(async (tx: typeof db) => {
    ...
    const eventDate = inquiry.eventDate.toISOString().slice(0, 10);
  ...
  if (result.notify) await db.insert(notifications).values({ userId: result.booking.customerId, type: "catering_booking_confirmation", title: "Catering booking ready to confirm", message: "Your provider has offered booking terms for your explicit confirmation.", linkUrl: "/services/catering#my-bookings" }).catch(() => undefined);
...
r.post("/bookings/:id/customer-confirm", requireAuth, async (req, res, next) => { try {
  ...
  await db.insert(notifications).values({ userId: updated.providerId, type: "catering_booking_confirmed", title: "Catering booking confirmed", message: "The customer explicitly accepted the booking terms.", linkUrl: "/services/catering/provider#bookings" }).catch(() => undefined);
...
r.post("/bookings/:id/complete", requireAuth, async (req, res, next) => { try {
  ...
  const updated = await db.transaction(async (tx: typeof db) => { const [row] = await tx.update(cateringBookings).set({ status: "completed", completedAt: now, updatedAt: now }).where(and(eq(cateringBookings.id, id), eq(cateringBookings.providerId, providerId), eq(cateringBookings.status, "confirmed"))).returning(); if (!row) return null; await tx.update(cateringReviews).set({ verifiedEvent: true, updatedAt: now }).where(and(eq(cateringReviews.inquiryId, row.inquiryId), eq(cateringReviews.providerId, row.providerId), eq(cateringReviews.reviewerId, row.customerId))); return row; });
  if (!updated) return res.status(409).json({ message: "Booking changed before completion finished" });
  await db.insert(notifications).values({ userId: updated.customerId, type: "catering_booking_completed", title: "Catering event marked complete", message: "Your provider recorded the event as complete. A linked review can now be verified.", linkUrl: "/services/catering#my-bookings" }).catch(() => undefined);
  res.json({ booking: serializeCateringBooking(updated) });
} catch (error) { if (error instanceof z.ZodError) return res.status(400).json({ message: error.issues[0]?.message }); next(error); } });
