...
// --- Keys ---
// Server key: IP-restricted; used only from server to Google
const GOOGLE_SERVER_KEY =
  process.env.GOOGLE_PLACES_SERVER_KEY ||
  process.env.GOOGLE_MAPS_API_KEY || // fallback for legacy name
  "";

// Browser key: HTTP referrer-restricted; sent to the client to load JS API
const GOOGLE_BROWSER_KEY = process.env.GOOGLE_MAPS_JS_KEY || "";
...
// ---------- Browser JS loader (returns ONLY the browser key) ----------
googleRouter.get("/maps-script", (_req, res) => {
  if (!GOOGLE_BROWSER_KEY) {
    return res.status(500).send("GOOGLE_MAPS_JS_KEY not configured");
  }
...
// ---------- Geocode helper (server key) ----------
async function geocode(near: string) {
  if (!GOOGLE_SERVER_KEY) throw new Error("missing_server_key");
  ...
  try {
    if (!GOOGLE_SERVER_KEY) {
      return sendJson(res, 500, {
        error: "missing_key",
        message: "GOOGLE_PLACES_SERVER_KEY (or GOOGLE_MAPS_API_KEY) not visible to server process.",
      });
    }
...
// ---------- Place Photo proxy (server key) ----------
googleRouter.get("/photo", async (req, res) => {
  try {
    if (!GOOGLE_SERVER_KEY) return res.status(500).send("Missing GOOGLE_PLACES_SERVER_KEY");
