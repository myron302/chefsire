...
let DATABASE_URL = (process.env.DATABASE_URL ?? "").trim();// /httpdocs/server/drizzle.config.ts  (pure ESM)
import { defineConfig } from "drizzle-kit";
import fs from "node:fs";
import { fileURLToPath } from "node:url";
import path from "node:path";
...
export default defineConfig({
  // 👇 Point to all modular schema files
  schema: "./server/db/schema/**/*.ts",
  // where drizzle-kit will emit migrations and SQL
  out: "./server/drizzle",
  dialect: "postgresql",
